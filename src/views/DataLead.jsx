...
const criteriaImg = '/assets/images/datalead/dataenrich-lead.jpeg';
const verifyImg = '/assets/images/datalead/baseunifiee-lead.jpeg';
const scoringImg = '/assets/images/datalead/segmentation-lead.jpeg';
const integrationImg = '/assets/images/datalead/datalead.jpeg';
const dataleadVideo = '/assets/video/data-lead.mp4';
...
const features = (img1, img2, img3, img4) => [
  ...
  {
    id: 2,
    title: 'Vérification humaine systématique',
    image: img2,
    description: 'Chaque contact est vérifié à la main : poste actuel, entreprise active, coordonnées valides. On garantit un taux de rebond minimal.',
  },
  {
    id: 3,
    title: 'Base scorée & Priorisée',
    image: img3,
    description: 'Le fichier intègre déjà une logique de priorisation (hot leads / warm leads) pour que vos commerciaux sachent exactement par qui commencer.',
  },
  {
    id: 4,
    title: 'Intégration CRM en 1 clic',
    image: img4,
    description: 'Livrable prêt à être injecté directement dans HubSpot, Pipedrive ou Salesforce avec tous les champs normalisés.',
  }
];
      ...
      <section style={{ 
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h2 style={{ fontSize: 'clamp(1.6rem, 2.6vw, 2.2rem)', fontWeight: 700, textAlign: 'center', color: '#1C2B27', marginBottom: '2.5rem', letterSpacing: '-0.02em', lineHeight: 1.2 }}>
            Ce que vous obtenez <span style={{ textDecoration: 'underline', color: '#1C2B27' }}>concrètement.</span>
          </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.4fr)', gap: '3rem', alignItems: 'start' }}>
                      ...
                      <div style={{ width: '100%', paddingRight: '1rem' }}>
                        ...
                        {isOpen && (
                          <div style={{ marginTop: '0.6rem', height: '3px', background: 'rgba(255,255,255,1.0)', width: '100%', borderRadius: '2px', overflow: 'hidden' }}>
                            <div key={`progress-${activeId}`} style={{ height: '100%', background: '#1F3A33', animation: 'drawProgressAccordionLead 4s linear forwards' }} />
                          </div>
                        )}
                      </div>
            ...
            <div style={{ position: 'sticky', top: '20vh', borderRadius: '20px', overflow: 'hidden', border: '5px solid rgba(28,43,39,0.14)', aspectRatio: '16/9', background: '#F6F3EC', boxShadow: '0 20px 40px -10px rgba(28,43,39,0.175)' }}>
              <img key={activeId} src={activeItem.image} alt={activeItem.title} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', display: 'block', animation: 'fadeInFeatureLead 0.4s ease' }} />
            </div>
          </div>
        </div>
        <style>{`
          @keyframes drawProgressAccordionLead { from { width: 0%; } to { width: 100%; } }
          @keyframes fadeInFeatureLead { from { opacity: 0; transform: scale(0.98); } to { opacity: 1; transform: scale(1); } }
        `}</style>
      </section>
    </div>
  );
...
export default function DataLead() {
  ...
  const smoothScroll = (e, targetId) => {
    ...
    window.requestAnimationFrame(function step(timestamp) {
      if (!start_point) start_point = timestamp;
      const progress = timestamp - start_point;
    ...
    <>
      <div style={{ backgroundColor: '#F6F3EC', color: '#1C2B27', minHeight: '100vh', paddingBottom: '4rem', fontFamily: 'var(--font-main)' }}>
        ...
        <section style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden' }}>
          ...
          {/* Fond pleine page */}
          <img src={fondDataLead} alt="" style={{
          ...
          <div style={{ position: 'relative', zIndex: 4, height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingLeft: '8%' }}>
            <p style={{ color: '#8A6D3B', fontWeight: 700, textTransform: 'uppercase', marginBottom: '1.2rem', letterSpacing: '0.12em', fontSize: '0.9rem' }}>Data Lead</p>
            <motion.h1 
              ...
              Pas assez de leads B2B&nbsp;?<br />
              On vous dit qui, quand<br />
              et pourquoi appeler.
            </motion.h1>
            <motion.div
              ...
              <span style={{ fontSize: '1.4rem', fontWeight: 800, color: '#8A6D3B' }}>27 %</span>
              <span style={{ fontSize: '0.85rem', color: 'rgba(28,43,39,0.6)', maxWidth: '280px', lineHeight: 1.3 }}>du temps des commerciaux perdu à vérifier des données obsolètes <span style={{ color: 'rgba(28,43,39,0.6)' }}>(Gartner)</span></span>
            </motion.div>
            <motion.p
              ...
              Chaque mois, nous vous livrons un flux de contacts prêts à appeler : nettoyés et enrichis par IA, puis vérifiés à la main par notre équipe. Vos commerciaux savent qui contacter, quand, et pourquoi.
            </motion.p>
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '10rem 2rem' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8rem', alignItems: 'start' }}>
              ...
              >
                ...
                <p style={{ color: '#8A6D3B', fontWeight: 700, textTransform: 'uppercase', marginBottom: '1rem' }}>Le constat</p>
                <h2 style={{ fontSize: 'clamp(1.6rem, 2.6vw, 2.2rem)', fontWeight: 700, color: '#1C2B27', marginBottom: '2rem' }}>La prospection à l'aveugle ne fonctionne plus.</h2>
              ...
              {/* Right Column (Static blocks) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem', paddingTop: '10rem' }}>
                {[
                  { title: "Nouveau segment", desc: "Votre base existante ne couvre pas la cible visée." },
                  { title: "Nouveau produit", desc: "Les acheteurs ne sont plus les mêmes que d'habitude." },
                  { title: "Aide au recrutement", desc: "Donnez à votre nouveau commercial un territoire prêt à l'emploi." },
                  { title: "Arpenter le marché", desc: "Vous avez épuisé vos contacts et devez chercher de nouveaux comptes." }
                ].map((item, idx) => (
        ...
        <section style={{ padding: '6rem 2rem' }}>
          ...
          <EnjeuxCarousel
            items={features(criteriaImg, verifyImg, scoringImg, integrationImg).map((f, i) => ({
              short: ['Critères', 'Vérification', 'Scoring', 'Intégration CRM'][i],
              title: f.title,
              desc: f.description,
              image: f.image,
        ...
        <section style={{ padding: '10rem 2rem', maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <p style={{ fontSize: '1.2rem', color: 'rgba(28,43,39,0.6)', maxWidth: '700px', marginInline: 'auto' }}>
              Le périmètre exact est cadré ensemble avant tout démarrage pour garantir l'impact business.
            </p>
          </div>
      ...
      <CtaFinalZoom
        ...
        title={<>Prêt à construire la base<br />qui va alimenter votre pipeline ?</>}
        description="Fini le temps perdu sur des fichiers génériques."
      />
      </div>
    </>
  );
